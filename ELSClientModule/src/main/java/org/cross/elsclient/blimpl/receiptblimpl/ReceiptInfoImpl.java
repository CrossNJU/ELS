...
public class ReceiptInfoImpl implements ReceiptInfo {
	...
	public ReceiptVO toVO(ReceiptPO po) {
		...
		case TOTALMONEYIN:
			...
			Receipt_TotalMoneyInVO totalMoneyInvo = new Receipt_TotalMoneyInVO(totalmoneyinpo.getTime(),
			...
			for (int i = 0; i < size; i++) {
				receipt_MoneyInVOs.add((Receipt_MoneyInVO) toVO(receipt_MoneyInPOs.get(i)));
			}
