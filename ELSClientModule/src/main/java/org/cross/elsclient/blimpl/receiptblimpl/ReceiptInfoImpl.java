...
public class ReceiptInfoImpl implements ReceiptInfo {
	...
	public ReceiptVO toVO(ReceiptPO po) {
		...
		case TRANS:
			...
			Receipt_TransVO transvo = new Receipt_TransVO(transpo.getNumber(), transpo.getTime(), goodslist,
					transpo.getCost(), transpo.getTransNum(), transpo.getVeNum(), transpo.getStartPlace(),
					transpo.getArrivePlace(), transpo.getObserver(), transpo.getDriver(), transpo.getPerNum());
			transvo.approveState = transpo.getApproveState();
			return transvo;
		case STOCKIN:
			Receipt_StockInPO stockinpo = (Receipt_StockInPO) po;
			Receipt_StockInVO stockinvo = new Receipt_StockInVO(stockinpo.getNumber(), stockinpo.getTime(),
					stockinpo.getOrderNum(), stockinpo.getDestination(), stockinpo.getStockNum(), stockinpo.getPerNum(),
					stockinpo.getOrgNum());
			stockinvo.approveState = stockinpo.getApproveState();
			return stockinvo;
		case STOCKOUT:
			Receipt_StockOutPO stockoutpo = (Receipt_StockOutPO) po;
			Receipt_StockOutVO stockoutvo = new Receipt_StockOutVO(stockoutpo.getNumber(), stockoutpo.getTime(), null,
					stockoutpo.getDestination(), stockoutpo.getTransType().toString(), stockoutpo.getTransNumber(),
		...
		case MONEYIN:
			...
			Receipt_MoneyInVO moneyinvo = new Receipt_MoneyInVO(moneyinpo.getTime(), moneyinpo.getMoney(),
					moneyinpo.getPerson(), moneyinpo.getNumber(), moneyingoods, moneyinpo.getOrgNum(),
					moneyinpo.getPerNum());
		...
		case MONEYOUT:
			...
			Receipt_MoneyOutVO moneyoutvo = new Receipt_MoneyOutVO(moneyoutpo.getNumber(), moneyoutpo.getTime(),
					moneyoutpo.getMoney(), moneyoutpo.getAccountNum(), moneyoutpo.getSenderNum(),
					moneyoutpo.getClause(), moneyoutpo.getComments(), moneyoutpo.getPerNum(), moneyoutpo.getOrgNum());
		...
		case TOTALMONEYIN:
			...
			Receipt_TotalMoneyInVO totalMoneyInvo = new Receipt_TotalMoneyInVO(totalmoneyinpo.getNumber(),
					totalmoneyinpo.getTime(), totalmoneyinpo.getAccountNum(), totalmoneyinpo.getMoney(), moneyins,
	...
	public ReceiptPO toPO(ReceiptVO vo) {
		...
		case TRANS:
			...
			Receipt_TransPO transpo = new Receipt_TransPO(transvo.number, transvo.type, transvo.time, transvo.orgNum,
					transvo.perNum, transvo.cost, transvo.transNum, transvo.vehicleNum, transvo.orgNum,
					transvo.arriveOrgID, transvo.observerName, transvo.driverName);
		...
		case STOCKIN:
			...
			Receipt_StockInPO stockinpo = new Receipt_StockInPO(stockinvo.number, stockinvo.type, stockinvo.time,
					stockinvo.orgNum, stockinvo.perNum, stockinvo.goodsNumber, stockinvo.stockAreaNum,
					stockinvo.targetOrgID);
			stockinpo.setApproveState(stockinvo.approveState);
			return stockinpo;
		case STOCKOUT:
			Receipt_StockOutVO stockoutvo = (Receipt_StockOutVO) vo;

			Receipt_StockOutPO stockoutpo = new Receipt_StockOutPO(stockoutvo.number, stockoutvo.type, stockoutvo.time,
					stockoutvo.orgNum, stockoutvo.perNum, stockoutvo.goodsNum, stockoutvo.targetOrgID,
					StringToType.toVehicleType(stockoutvo.vehicle), stockoutvo.transNumber);
			stockoutpo.setApproveState(stockoutvo.approveState);
			return stockoutpo;
		case MONEYIN:
			Receipt_MoneyInVO moneyinvo = (Receipt_MoneyInVO) vo;
			// totalMoneyIn 暂时是NULL
			Receipt_MoneyInPO moneyinpo = new Receipt_MoneyInPO(moneyinvo.number, moneyinvo.type, moneyinvo.time,
					moneyinvo.orgNum, moneyinvo.perNum, moneyinvo.money, null, moneyinvo.person);
			moneyinpo.setApproveState(vo.approveState);
			return moneyinpo;
		case MONEYOUT:
			Receipt_MoneyOutVO moneyoutvo = (Receipt_MoneyOutVO) vo;
			Receipt_MoneyOutPO moneyoutpo = new Receipt_MoneyOutPO(moneyoutvo.number, moneyoutvo.type, moneyoutvo.time,
					moneyoutvo.orgNum, moneyoutvo.perNum, moneyoutvo.money, moneyoutvo.receiveID, moneyoutvo.clause,
		...
		case TOTALMONEYIN:
			...
			Receipt_TotalMoneyInPO totalMoneyInpo = new Receipt_TotalMoneyInPO(totalmoneyinvo.number,
	...
	public ArrayList<ReceiptVO> findByTimeAndType(ReceiptType type, String start, String end) throws RemoteException {
		ArrayList<ReceiptPO> polist = receiptdata.findByTimeAndType(start, end, type);
	...
	public ArrayList<Receipt_MoneyInVO> findByTotalMoneyIn(String totalmoney) throws RemoteException {
		ArrayList<ReceiptPO> orders = receiptdata.findByType(ReceiptType.MONEYIN);
