...
public class DriverManagePanel extends ELSManagePanel{
	...
	ArrayList<PersonnelVO> drivers;
	DriverManageTable list;
	ELSButton addBtn;
	
	public DriverManagePanel(){}
	
	public DriverManagePanel(PersonnelBLService personnelbl) {
		super();
	...
	public void setContentPanel() {
		...
		list= new DriverManageTable(s,itemWidth,personnelbl);
		...
		public void mouseClicked(MouseEvent e) {
			...
			if(((String)modeBox.getSelectedItem()).equals("按司机姓名查询")){
				if(e.getSource()==searchBtn){
					...
					try {
						drivers = personnelbl.findByName(name);
					} catch (RemoteException e1) {
						// TODO Auto-generated catch block
						e1.printStackTrace();
					}
					list.init();
					for (PersonnelVO  driver: drivers) {
						list.addItem(driver);
					}
				}else if (e.getSource() == addBtn){
					DriverAddPanel driveradd = new DriverAddPanel(personnelbl);
